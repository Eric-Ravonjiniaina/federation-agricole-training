    ...
    public ResponseEntity<?> createMembers(@RequestBody List<MemberDTO> dtos) {
        try {
            ...
            for (MemberDTO dto : dtos) {
                MemberEntity entity = service.create(dto);
                created.add(CollectivityMapper.toMemberDTO(entity));
            }
            return ResponseEntity.status(HttpStatus.CREATED).body(created);
        } catch (BadRequestException e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(e.getMessage());
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Collectivity or Member not found: " + e.getMessage());
        }
    }
}
